...
from .registry import SIS_HEAD_REG
...
class LookByMultiQ(nn.Module):
    ...
    def forward(self, q, qpe, feat, feat_pe, mask_feature):
        """
        q, qpe: B, nq, C
        ...
        q = self.norm1(q + self.dropout1(self.cross_attn(q=q + qpe, k=feat + feat_pe, v=feat)))
...
@SIS_HEAD_REG.register()
class TransformerDecoder(nn.Module):
    @configurable
    def __init__(self, num_queries=100, embed_dim=256, num_heads=8, hidden_dim=1024, dropout_attn=0.0, dropout_ffn=0.0,
                 num_blocks=3, key_features=["res5", "res4", "res3"], mask_key="res2"):
        super().__init__()
    ...
    def forward(self, feats, feats_pe):
        ...
        for idx in range(self.num_blocks):
            ...
            q = self.lookByMultiQ_layers[idx](q, qpe, feat, feat_pe, mask_feature=mask_feature)
