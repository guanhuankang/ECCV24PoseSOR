...
from ..other import init_weights_, LayerNorm2D
...
class BiFRC(nn.Module):
    def __init__(self, dim=256, feat_keys=["res2","res3","res4","res5"]):
        super().__init__()
        self.feat_keys = feat_keys
        self.frcs = nn.ModuleList([FRC(dim, dim) for _ in range(len(feat_keys)-1)])
        self.convs = nn.ModuleList([
            nn.Sequential(nn.Conv2d(dim, dim, 1), nn.GELU(), LayerNorm2D(dim)) 
            for _ in range(len(feat_keys)-1)
        ])
    
    def forward(self, feats):
        feats = [feats[k] for k in self.feat_keys][::-1]  ## 5->2
        for i, frc in enumerate(self.frcs):
            feats[i+1] = frc(high_feat=feats[i], low_feat=feats[i+1])
        feats = feats[::-1]  ## 2->5
        for i, conv in enumerate(self.convs):
            feats[i+1] = conv(
                F.interpolate(feats[i], size=feats[i+1].shape[2::], mode="bilinear") + feats[i+1]
            ) + feats[i+1]
        feats = dict((k, v) for k, v in zip(self.feat_keys, feats))
        return feats

@NECK_HEAD.register()
class BiFrcPN(nn.Module):
    '''
    ...
    def __init__(self, dim=256, feat_dims=(128,256,512,1024), feat_keys=["res2","res3","res4","res5"]):
        ...
        self.lateral_conv = nn.ModuleDict(dict(
            (key, nn.Sequential(nn.Conv2d(d_in, dim, 1), nn.GELU(), LayerNorm2D(dim)))
            for key, d_in in zip(feat_keys, feat_dims)
        ))
        self.bifrcs = nn.ModuleList([BiFRC(dim=dim, feat_keys=feat_keys) for _ in range(3)])
        self.feat_keys = feat_keys
    ...
    def forward(self, feats):
        ...
        """
        feats = dict( (k, self.lateral_conv[k](v)) for k, v in feats.items())
        for layer in self.bifrcs:
            feats = layer(feats)
        return feats
