...
class PDRRI(nn.Module):
    ...
    def forward(self, q, qpe, pose, pose_pe, feats, feats_pe, q_mask=None):
        ...
        mat_repr = self.share_mlp(torch.cat([
            q.unsqueeze(2).repeat_interleave(n, dim=2),
            q.unsqueeze(1).repeat_interleave(n, dim=1),
        ], dim=-1))  ## B, n, n, C
        sha_repr = torch.mean(mat_repr, dim=2)  ## B, n, C !!!
        q = self.agg_head(torch.cat([q, q, sha_repr], dim=-1))
        nxt = self.nxt_head(torch.cat([q, q, sha_repr], dim=-1))
